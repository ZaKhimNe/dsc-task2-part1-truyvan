...
# ---------------------------------------------------------------------------
# BANG TRA tieu de + ham pair_text — CHEP NGUYEN VAN tu kernel T12
# (kaggle_layer3/rerank_title_input_notebook.py). KHONG viet lai: neu hai ban
# lech nhau mot chi tiet thi con so +0,0121 do tren T12 khong con ap dung duoc
# cho lan chay nay.
# ---------------------------------------------------------------------------
uid_to_title = {}
n_no_title = n_eq = 0
...
n_hit = sum(1 for us in baskets.values() for u in us if uid_to_title.get(u["unit_id"]))
n_tot = sum(len(us) for us in baskets.values())
print(f"  bang tra tieu de: {len(uid_to_title)} unit_id", flush=True)
print(f"  ung vien THUC SU doi chu: {n_hit}/{n_tot} = {n_hit/n_tot:.1%}", flush=True)
assert n_hit / n_tot > 0.5, "Duoi 50% ung vien co tieu de dung duoc"
...
pairs, spans, bare_texts = [], [], []
for qid in qids:
    us = baskets[qid]
    spans.append((qid, len(pairs), len(us)))
    for u in us:
        pairs.append([qa[qid]["question"], pair_text(u, "rr_title")])
        bare_texts.append(u["text"])
n_diff = sum(1 for k in range(len(pairs)) if pairs[k][1] != bare_texts[k])
print(f"  cap co chu KHAC ban tran: {n_diff}/{len(pairs)} = {n_diff/len(pairs):.1%}", flush=True)

# Dem cap vuot 512 token cho CA HAI bien the — CrossEncoder(max_length=512) cat
# phan thua. Neu gan tieu de day nhieu cap qua nguong thi phan duoi Khoan bi cat,
# va loi cua tieu de se bi tra gia bang noi dung mat di.
_tok = model.tokenizer
_q = [qa[qid]["question"] for qid, st, n in spans for _ in range(n)]
for _lbl, _txt in (("tran", bare_texts), ("tieu de + Khoan", [x[1] for x in pairs])):
    _n = sum(1 for a, b in zip(_q, _txt)
             if len(_tok(a, b, truncation=False)["input_ids"]) > 512)
    print(f"  cap vuot 512 token ({_lbl}): {_n}/{len(_txt)} = {_n/len(_txt):.1%}", flush=True)
...
out_path = OUT_DIR / f"L3_rerank_private_{ARM}_rrtitle.jsonl"
